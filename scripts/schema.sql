...
ALTER TABLE interns ADD INDEX idx_intern_status (status);
ALTER TABLE vacancies ADD INDEX idx_vacancy_status (status);
ALTER TABLE logbooks ADD INDEX idx_logbook_status (status);

-- Foreign Keys (using ON DELETE CASCADE or SET NULL based on safe defaults)
-- Note: users(linked_id) must be indexed to be referenced.
ALTER TABLE users ADD INDEX idx_linked_id (linked_id);
ALTER TABLE vacancies ADD FOREIGN KEY (company_id) REFERENCES users(linked_id) ON DELETE CASCADE;
ALTER TABLE applicants ADD FOREIGN KEY (vacancy_id) REFERENCES vacancies(id) ON DELETE CASCADE;
ALTER TABLE applications ADD FOREIGN KEY (vacancy_id) REFERENCES vacancies(id) ON DELETE CASCADE;
ALTER TABLE logbooks ADD FOREIGN KEY (intern_id) REFERENCES interns(id) ON DELETE CASCADE;
